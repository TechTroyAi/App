...
v1.4.4 adds twenty-one approved project-specific image-model assets across three bounded production batches. Batch 01 contains ten HUD controls (play, continue, challenge, sound on/off, pause, reforge, confirm, back, and menu). Batch 02 contains a fortress-forge menu scene, a unified title logo, three full action plates, and Best/Wave/Daily record emblems. Batch 03 adds complete Best, Daily, and Wave record plates with clean runtime-value fields; three base generations and three cleanup refinements stayed within that session's ten-image limit. The retained high-resolution outputs and reviewed Android derivatives live under `artwork/ui-production/v1.4.4/`; `tools/process_hud_sprites_v144.sh` records the deterministic isolation and publishing step. They were generated specifically for Blockhold Defense and contain no third-party sprite-pack pixels.

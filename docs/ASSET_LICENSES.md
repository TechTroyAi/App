...
v1.4.4 adds eighteen project-specific image-model concepts across two bounded production batches. Batch 01 contains ten HUD controls (play, continue, challenge, sound on/off, pause, reforge, confirm, back, and menu). Batch 02 contains a fortress-forge menu scene, a unified title logo, three full action plates, and Best/Wave/Daily record emblems. The high-resolution outputs and reviewed Android derivatives are retained under `artwork/ui-production/v1.4.4/`; `tools/process_hud_sprites_v144.sh` records the deterministic isolation and publishing step. They were generated specifically for Blockhold Defense and contain no third-party sprite-pack pixels.

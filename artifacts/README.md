...
## Windows: Aureus.exe (screen recorder, made by Troy)

**`Aureus-v1.2.0.exe`** is the current Windows screen-recording utility — a
single portable executable, no installer and no runtime dependencies, themed
with a **black & gold minimal console UI** (gold accents, pulsing REC dot; set
`NO_COLOR=1` for plain output). Since v1.2.0 it also opens a **black & gold
browser studio** on localhost (record, browse, **trim** and delete clips), and
you can pick **where recordings are saved** with `-outdir` (default: your
Videos folder). It carries the custom **black & gold app icon**, Windows
**version info** ("Made by Troy", product *Aureus*), a DPI-aware manifest, and
is **code-signed** with the project's self-signed certificate. Press `F9` to
start/stop recording, `ESC` to quit; writes an animated GIF by default (MP4
too if ffmpeg is installed). Full usage and flags:
[`aureus/README.md`](../aureus/README.md).

| File | SHA-256 | Status |
|---|---|---|
| `Aureus-v1.2.0.exe` | `a7d3d02b5abc4c35fa2b03bf554ec50ffe4ed2adfab80fafd44b6db65e84a68b` | **current** — browser studio (record/browse/trim/delete), `-outdir`, `-trim`; icon + "Made by Troy" metadata; self-signed |
| `Aureus-v1.1.0.exe` | `299a88a9374395ea458df39a0e7cb34f1985728f6c2176e5bf4f297f4561ab12` | superseded — renamed from ScreenRecorder; icon + "Made by Troy" metadata; self-signed |
...
- Size: ~7.2 MB (v1.2.0, includes the embedded browser studio + icon) ·
  Format: PE32+ executable for MS Windows, x86-64 (machine `0x8664`)
- Icon + version info + manifest are embedded at build time from the committed
  `aureus/resource.syso` (generated by `goversioninfo` from
  `aureus/versioninfo.json`, the icon in `artwork/aureus/aureus.ico`, and the
  manifest `aureus/aureus.exe.manifest`).
- Signing identity: `CN=Aureus, OU=Aureus Release, O=Made by Troy, L=Davao City, ST=Davao Region, C=PH`
- Signing cert SHA-256: `62:19:1b:dd:ad:b8:5a:90:a7:e7:23:56:51:c0:8c:16:ec:2a:0c:bd:1c:01:a5:b9:28:94:d4:9a:18:4b:cf:d2`
  (self-signed, RSA 4096, EKU *Code Signing*, valid 2026–2036; keystore at
  `.signing/aureus-codesign.p12`). **Self-signed ≠ trusted:** Windows will still
  show SmartScreen "More info → Run anyway"; only a certificate chaining to a
  public CA removes that.
...
- Source: [`aureus/`](../aureus/) — cross-compiles from any OS; tests run on any OS
- CI: `.github/workflows/windows-exe.yml` rebuilds it on a native
  `windows-latest` runner on every change to `aureus/**`, signs it with
  `signtool`, and re-commits the fresh binary here
